...
import { Plane, Hotel, Home, Train, Bus, Car, Palmtree, CreditCard, MapPin, Search } from 'lucide-react';
...
const HomeSearch = () => {
    ...
    const handleSearchClick = () => {
        if (activeTab === 'Hotels' || activeTab === 'Homestays') {
            if (hotelQuery.trim()) {
                navigate(`/hotels?location=${encodeURIComponent(hotelQuery)}`);
            } else {
                navigate('/hotels');
            }
        } else {
    ...
    const renderSearchFields = () => {
        switch (activeTab) {
            ...
            case 'Homestays':
                return (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-0 box-border border rounded-xl overflow-visible shadow-sm relative z-50">
                        <div className="p-4 hover:bg-blue-50 cursor-pointer border-r relative group transition-all md:col-span-2">
                            <span className="text-xs text-gray-500 font-bold uppercase block mb-1">City, Hotel, or Location</span>
                            <input
                                type="text"
                                value={hotelQuery}
                                onChange={(e) => setHotelQuery(e.target.value)}
                                placeholder="Enter city (e.g. Goa, Manali, Udaipur)..."
                                className="text-xl font-black text-gray-800 block w-full bg-transparent border-none outline-none placeholder-gray-300"
                            />
                            <span className="text-xs text-sky-600 font-bold block mt-1">Stayinn Luxury Resorts & Villas</span>
                        </div>
                        ...
                        <div className="p-4 hover:bg-blue-50 cursor-pointer border-r relative group transition-all">
                            ...
                            <span className="text-xl font-black text-gray-800 block">Today</span>
                        ...
                        <div className="p-4 hover:bg-blue-50 cursor-pointer relative group transition-all">
                            <span className="text-xs text-gray-500 font-bold uppercase block mb-1">Guests & Rooms</span>
                            <span className="text-xl font-black text-gray-800 block">2 Guests, 1 Room</span>
                            <span className="text-xs text-gray-500 block">Couples & Family Friendly</span>
                        </div>
                    </div>
                );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-0 box-border border rounded-xl overflow-visible shadow-sm relative z-50">
                        ...
                        <div className="p-4 hover:bg-blue-50 cursor-pointer border-r relative group transition-all md:col-span-2">
                            ...
                            <span className="text-xs text-gray-500 truncate block w-full">
                                {query ? "Searching for " + query : "Search for places, packages..."}
                            </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-0 box-border border rounded-xl overflow-hidden shadow-sm">
                        <div className="p-4 hover:bg-blue-50 cursor-pointer border-r relative group transition-all md:col-span-4">
                            <div className="flex items-center justify-center p-8 text-gray-400">
                                <span className="text-lg">Select 'Hotels', 'Flights', or 'Holiday Packages' to search</span>
                            </div>
                        </div>
                    </div>
                );
                    ...
                    {tabs.map((tab) => (
                        <button
                            ...
                            onClick={() => {
                                ...
                                if (tab.name === 'Hotels' || tab.name === 'Homestays') {
                                    // Smooth tab selection for hotels
                                }
                            }}
